...
echo -e "${BBLUE}Installing dependencies, ${BYELLOW}it requires root access!"

DISTRO_ID=$(grep -w ID /etc/os-release | awk -F= '{gsub(/"/, "", $2); print $2}')

echo -e "${BBLUE}System:"
if [ "$DISTRO_ID" = "debian" ] || [ "$DISTRO_ID" = "ubuntu" ]; then
    echo -e "  ${BGREEN}Debian family ( Debian, Ubuntu, Raspberry Pi OS ... )"
    ...
    echo -e "  ${BGREEN}Red Hat family ( Fedora, RHEL, CentOS ... )"
    sudo dnf update -y &&
